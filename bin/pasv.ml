(* If you pass a fasta and it doesn't look like an alignment hmmer will give
   error code 7. Probably shouldn't rely on the error code tho.

   If you pass a hmm file that is bad it also gives an error (code 1). the hmm
   tag is HMMER3/f and should be the first thing in the file. *)
...
let common_opts_term =
  let make_common_opts outdir roi_start roi_end force verbosity :
      Pasv.common_opts =
    {
      ...
      roi_start = Option.map roi_start ~f:Position.one_raw_of_int;
      roi_end = Option.map roi_end ~f:Position.one_raw_of_int;
      force;
      verbosity;
    }
  in
...
let pasv_check_term =
  let check_opts_term =
    let make_check_opts alignment key_residues : Pasv.Check.opts =
      { alignment; key_residues = Position.List.one_raw_of_list key_residues }
    in
...
let parse_cli () =
  match Term.eval_choice pasv_root_cmd subcommands with
